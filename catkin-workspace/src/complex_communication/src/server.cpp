...
bool TicTacToe::applyTurn(unsigned spot, int current_player)
{
  if (nextPlayer_ == current_player)
  {
    ...
    if (board_[spot] == 0)
    {
      ...
      {
        nextPlayer_ = symbols_[1];
      }
      else if (current_player == symbols_[1])
      {
        nextPlayer_ = symbols_[0];
      }
...
void TicTacToe::announceBoard() const
{
  ...
  ROS_INFO("** Next player %d", nextPlayer_);
  ROS_INFO("** Free spots: %d", freeSpots_);
  for (int i = 0; i < boardSize_; i++)
  {
    if ((i + 1) % 3 != 0)
    {
